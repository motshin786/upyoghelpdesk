...
<tr id="reply-<?php echo esc_attr( the_ID() ); ?>" class="wpas-reply-single wpas-status-<?php echo esc_attr( get_post_status() ); ?> wpas_user_<?php echo esc_attr( $user_role ); ?>" valign="top">
	...
	<?php else: ?>
		...
		<td><strong class="wpas-profilename"><?php echo wp_kses_post(apply_filters('wpas_fe_template_detail_reply_display_name', $user->data->display_name, $post )); ?></strong>
			...
			?>
			<p style="text-align: center; margin-bottom: 0px !important;"><?php echo force_balance_tags( $content_reply ); ?></p>
