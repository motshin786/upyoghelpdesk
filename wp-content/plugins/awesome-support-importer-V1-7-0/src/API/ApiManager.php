...
class ApiManager implements ApiManagerInterface
{
    ...
    protected function getClassname($selectedApi)
    {
        $classes = [
            ...
            'freshdesk'  => '\Provider\Freshdesk\ServiceProvider',
        ];
    ...
    protected function initData(array $data)
    {
        ...
        foreach ($times as $key => $time) {
            ...
            if (array_key_exists($key, $data) && $data[$key]) {
                $date = date_create_from_format(get_option('date_format'), $data[$key]);
                $data[$key] = $this->toFormattedDate($date->format('Y-m-d'), 'Y-m-d');
                $data[$key] .= $time;
            }
        }
