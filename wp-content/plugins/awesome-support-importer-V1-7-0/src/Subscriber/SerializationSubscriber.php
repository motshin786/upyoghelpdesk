...
class SerializationSubscriber extends AbstractSubscriber
{
    ...
    {
        $start = date_create_from_format(get_option('date_format'), $start);
        $end = date_create_from_format(get_option('date_format'), $end);
        $isInvalidRange = empty($end) ? false : ($end->getTimestamp() < $start->getTimestamp());
        update_option(
